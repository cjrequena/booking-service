...
import com.cjrequena.sample.command.handler.domain.model.command.ExpireBookingCommand;
...
@Component
@Transactional
public class ExpireBookingCommandHandler extends CommandHandler<ExpireBookingCommand> {
  ...
  public Aggregate handle(@Nonnull Command command) {
    ...
    if (!(command instanceof ExpireBookingCommand)) {
      throw new IllegalArgumentException("Expected command of type ExpireBookingCommand but received " + command.getClass().getSimpleName());
    }
  ...
  @Nonnull
  @Override
  public Class<ExpireBookingCommand> getCommandType() {
    return ExpireBookingCommand.class;
  }
