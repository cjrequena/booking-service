...
@Component
@Transactional
public class CreateBookingCommandHandler extends CommandHandler<CreateBookingCommand> {
  ...
  public Aggregate handle(@Nonnull Command command) {
    ...
    if (!(command instanceof CreateBookingCommand)) {
      throw new IllegalArgumentException("Expected command of type CreateBookingCommand but received " + command.getClass().getSimpleName());
    }
